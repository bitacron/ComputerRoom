...
/*******************************************************************************
* 函数名：LDR_GetDigital
* 描述  ：读取光敏电阻数字量（与 MQ2/Flame 一致：1=条件成立）。
* 输出  ：0 = 光照正常（高于阈值），1 = 光照过低（低于阈值，DO 低电平有效）
*******************************************************************************/
uint8_t LDR_GetDigital(void)
{
	// 常见模块：光照低于阈值时 DO 输出低电平
	return (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_13) == Bit_RESET) ? 1 : 0;
}
