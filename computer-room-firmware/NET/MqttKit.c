...
uint8 MQTT_UnPacketRecv(uint8 *dataPtr)
{
    ...
    uint8 len_bytes = MQTT_ReadLength(dataPtr + 1, 4, &remain_len);
		
		msgPtr = dataPtr + len_bytes + 1;

		if(remain_len < 2 || dataPtr[0] & 0x01)					//retain
			return 255;
		
		uint16 topic_len = ((uint16)msgPtr[0] << 8 | msgPtr[1]);
		...
		{
			status = MQTT_PKT_CMD;
			UsartPrintf(USART_DEBUG, "status is MQTT_PKT_CMD \r\n");
		}
		else{
			status = MQTT_PKT_PUBLISH;
			UsartPrintf(USART_DEBUG, "status is MQTT_PKT_PUBLISH \r\n");
		}
	}
	else
		status = type;
 ...
 * @param cmdid 输出参数,指向分配的报文主题topic字符串(调用者需使用MQTT_FreeBuffer函数释放)
 * @param req 输出参数,指向分配的负载数据(可能不以'\0'结尾,但本函数会添加结束符,方便字符串处理)
 * @param req_len 输出参数,负载数据的长度(不含添加的结束符)
 * @return 0成功,非0失败
 */
uint8 MQTT_UnPacketCmd(uint8 *rev_data, int8 **cmdid, int8 **req, uint16 *req_len)
{
    ...
    // 检查参数
    if (rev_data == NULL || cmdid == NULL || req == NULL || req_len == NULL)
        return 1;

    // 剩余解析长度(MQTT_ReadLength应返回实际占用的字节数，并填充remain_len)
    len_bytes = MQTT_ReadLength(rev_data + 1, 4, &remain_len);
    if (len_bytes == 0 || remain_len == 0)
        return 2;   // 剩余长度解析失败

    // 检查剩余长度是否合理 防止越界
    if (remain_len > 1024)   // 可根据实际情况自己调整上限
        return 3;

    // 指向可变头开始位置(固定头1字节 + 剩余长度字节数)
    ptr = rev_data + 1 + len_bytes;

    // 主题长度2字节(MSB, LSB)
    if (remain_len < 2)
        return 4;   // 剩余长度不足2子节,无法读取主题长度
    topic_len = ((uint16)ptr[0] << 8) | ptr[1];

    // 检查剩余长度是否包含完整的主题和至少0字节负载
    if (remain_len < 2 + topic_len)
        return 5;   // 数据不完整

    // 分配内存给主题（加1字节用于结束符）
    *cmdid = (int8 *)MQTT_MallocBuffer(topic_len + 1);
    if (*cmdid == NULL)
        return 6;    // 内存分配失败
    ...
    *req_len = remain_len - 2 - topic_len;
    ...
    {
        // 负载长度为0，可分配一个字节存放结束符，或直接置NULL
        *req = (int8 *)MQTT_MallocBuffer(1);
        if (*req == NULL)
        {
            MQTT_FreeBuffer(*cmdid);
            *cmdid = NULL;
