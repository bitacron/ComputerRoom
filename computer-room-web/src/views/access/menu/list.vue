<template>
  <div class="app-container">
    <el-input v-model="filterText" placeholder="按名称筛选" style="margin-bottom:30px;"/>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="dialogFormValue">
      <el-form ref="menu" :model="menu" :rules="menuValidateRules" label-width="120px">
        ...
        <el-form-item label="排序" prop="sort">
          <el-input-number v-model="menu.sort" :min="0" :step="1" controls-position="right"/>
        </el-form-item>
    ...
    <el-dialog :visible.sync="dialogPermissionVisible" title="添加功能">
      <el-form ref="permission" :model="permission" :rules="permissionValidateRules" label-width="120px">
        ...
        <el-form-item label="排序">
          <el-input-number v-model="permission.sort" :min="0" :step="1" controls-position="right"/>
        </el-form-item>
...
export default {
  ...
  watch: {
    filterText(val) {
      ...
      this.menuList = this.filterMenuTree(this.allMenuList, keyword)
    }
  },
  ...
  methods: {
    filterMenuTree(list, keyword) {
      if (!list || list.length === 0) {
        return []
      }
      const result = []
      list.forEach(node => {
        const children = node.children ? this.filterMenuTree(node.children, keyword) : []
        const nameMatch = (node.name || '').toLowerCase().indexOf(keyword) !== -1
    ...
    fetchNodeList() {
      menu.getNestedTreeList().then(response => {
        if (response.code === 20000) {
          this.allMenuList = response.data.children || []
          const keyword = (this.filterText || '').trim().toLowerCase()
          this.menuList = keyword
            ? this.filterMenuTree(this.allMenuList, keyword)
            : this.allMenuList
        } else {
