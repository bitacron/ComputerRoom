...
export default {
  ...
  methods: {
    ...
    // 初始化图表（柱状图配置 + dataZoom）
    initCharts() {
      // 温度+湿度柱状图（双柱）
      this.tempHumChart = echarts.init(this.$refs.tempHumChart)
      this.tempHumChart.setOption({
        title: { text: '温度与湿度统计', left: 'center' },
        tooltip: { trigger: 'axis' },
        legend: { data: ['温度(℃)', '湿度(%)'], top: 30 },
        xAxis: { type: 'category', data: [] },
        yAxis: { type: 'value', name: '数值' },
        series: [
          { name: '温度(℃)', type: 'bar', data: [], barWidth: '40%' },
          { name: '湿度(%)', type: 'bar', data: [], barWidth: '40%' }
        ],
        dataZoom: [
      ...
      this.gasChart.setOption({
        title: { text: '烟雾浓度统计', left: 'center' },
        tooltip: { trigger: 'axis' },
        xAxis: { type: 'category', data: [] },
        yAxis: { type: 'value', name: '浓度(ppm)' },
        series: [
          { name: '烟雾浓度(ppm)', type: 'bar', data: [], barWidth: '50%' }
        ],
        dataZoom: [
          { type: 'slider', start: 0, end: 100 },
          { type: 'inside', start: 0, end: 100 }
